...
public:
  ...
  void run(){
    float angle = velocities_.steering_angle;
    float vel = velocities_.speed;
  ...
  {
    const_angle = constrain(input, this.minInAngle, this.maxInAngle);
    if (const_angle >= 0){
      pwm_value = this.neutralPWMAngle - (const_angle/this.maxInAngle)* this.maxOutDifAngle;
    }
    else {
      pwm_value = this.neutralPWMAngle + (const_angle/this.maxInAngle)* this.minOutDifAngle;
    }
    return pwm_value;
  }
  ...
  {
    const_vel = constrain(input, this.minInVel, this.maxInVel);
    pwm_value = this.neutralPWMAngle + this.minOutDifVel + (const_angle/this.maxInAngle) * this.maxOutDifVel;
    return pwm_value;
  }
